...
class Trainer(object):
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if self.is_data_parallel_master:  # only save one checkpoint
            ...
            '''
            if 'optok' in self.args.arch:
                if not self.args.optok_normal_enc:
                    self.model.encoder.optokEnc.saveNLMasMLM(filename+'.optok.enc.mlm')
                if not self.args.optok_normal_dec:
                    self.model.decoder.optokDec.saveNLMasMLM(filename+'.optok.dec.mlm')
            '''
        ...
        for i, sample in enumerate(samples):
            if 'optok' in self.args.arch:
                if 'net_input' in sample:
                    # make raw-samples before it goes to GPU 
                    raw_sample = self._get_raw_sample(sample)
                else:
                    # for dummy batch
                    raw_sample = self._get_raw_sample(self._dummy_batch)

            if hasattr(self.model, 'sp_src') or hasattr(self.model, 'sp_tgt'):
                if hasattr(self.args, 'optok_normal_enc') or hasattr(self.args, 'optok_normal_dec'):
                    sampling_src = hasattr(self.model, 'sp_src') and self.args.optok_normal_enc
                    sampling_tgt = hasattr(self.model, 'sp_tgt') and self.args.optok_normal_dec
                else:
                    sampling_src = hasattr(self.model, 'sp_src')
                    sampling_tgt = hasattr(self.model, 'sp_tgt')
                
                if 'net_input' in sample:
                    sample = self._sp_encode(sample, 
                                             sampling_src,
                                             sampling_tgt)

            # samples go to GPU here
            ...
            if 'optok' in self.args.arch:
                # add raw_sample to sample
                sample['net_input']['raw'] = raw_sample
    ...
    def _get_raw_sample(self, sample, boundarySymbol=''):
        def filter(dic, line):
            return (line != dic.bos_index) & (line != dic.eos_index) & (line != dic.pad_index)

        # src
        src_net_input = sample['net_input']['src_tokens'].numpy()
        src_raw_sample = [boundarySymbol.join(list(map(lambda x:self.task.src_dict[x], line[filter(self.task.src_dict, line)]))) 
                          for line in src_net_input]
        # tgt
        tgt_sample = sample['target'].numpy()
        tgt_raw_sample = [boundarySymbol.join(list(map(lambda x:self.task.tgt_dict[x], line[filter(self.task.tgt_dict, line)]))) 
                          for line in tgt_sample]

        # normalize
        src_raw_sample = [unicodedata.normalize('NFKC', line) for line in src_raw_sample]
        tgt_raw_sample = [unicodedata.normalize('NFKC', line) for line in tgt_raw_sample]

        return {'src': src_raw_sample, 'tgt': tgt_raw_sample}

    def _sp_encode(self, sample, src_sampling, tgt_sampling):
        def pad(ids, dic):
        ...
        raw_sample = self._get_raw_sample(sample)
        alpha = self.args.sp_alpha if hasattr(self.args, 'sp_alpha') else 0.2 
        if src_sampling:
            pieces = [self.model.sp_src.sample_encode_as_pieces(line.replace('▁', ' '), -1, alpha)
                        for line in raw_sample['src']]
            ids = [[self.task.src_dict.index(p) for p in pp] for pp in pieces]
            ids, ls = pad(ids, self.task.src_dict)
        ...
        if tgt_sampling:
            pieces = [self.model.sp_tgt.sample_encode_as_pieces(line.replace('▁', ' '), -1, alpha)
                        for line in raw_sample['tgt']]
            ids = [[self.task.tgt_dict.index(p) for p in pp] for pp in pieces]
            ids, ls = pad(ids, self.task.tgt_dict)
            ids_prev = [[self.task.tgt_dict.eos_index]+ii[:-1] for ii in ids]
            sample['net_input']['prev_output_tokens'] = torch.LongTensor(ids_prev)
            sample['target'] = torch.LongTensor(ids)
            # set to sample
