    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def forward(
        ...
        if self.encoder_attn is not None:
            ...
            #print('modules/transformer_layer.py, encoder_out, encoder_padding_mask, shape')
            ...
            x, attn = self.encoder_attn(
                ...
                opTokM=opTokM, # size of candidates of OpTok
                opTokAttn=opTokAttn, # Optok's probability for each tok cands.
            )
            x = self.dropout_module(x)
            x = residual + x
            if not self.normalize_before:
