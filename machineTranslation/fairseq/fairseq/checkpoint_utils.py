...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    if checkpoint_conds["checkpoint_best{}.pt".format(suffix)]:
        if 'optok' in trainer.args.arch:
            if not trainer.args.optok_normal_enc:
                trainer.model.encoder.optokEnc.saveNLMasMLM(
                    os.path.join(args.save_dir, "checkpoint_best{}.optok.enc.mlm".format(suffix)))
            if not trainer.args.optok_normal_dec:
                trainer.model.decoder.optokDec.saveNLMasMLM(
                    os.path.join(args.save_dir, "checkpoint_best{}.optok.dec.mlm".format(suffix)))
